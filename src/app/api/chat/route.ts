...
const ChatResponseSchema = z.object({
  thoughts: z.array(z.string().describe('A reflective thought on the current stream frame')).min(MIN_THINKING).max(MAX_THINKING).describe('Array of reflective thoughts describing observed frames'),
...
}).describe('Schema for validating the model’s chat response JSON');
      ...
      const summaryMessages = [
        ...
        },
        {
          ...
          content: [
            {
              type: 'text',
              text: `Please summarize the events in the following ${indices.length} frames:`
            },
            ...indices.map((idx) => ({
              type: 'image_url',
